...
{
    /// <summary>
    ///     Fetches scores for <see cref="LeaderboardType.Global"/> using API v2.
    /// </summary>
    public class ScoreFetcherGlobal : IScoreFetcher
    {
        public FetchedScoreStore Fetch(Map map, CancellationToken token = default)
        {
            try
            {
                ...
                var onlineScoresTask = System.Threading.Tasks.Task.Run(() => client.RetrieveScoreboardV2(map.Md5Checksum, OnlineScoreboard.Global));
                ...
                if (mapInfo?.Map != null)
                {
                    ...
                    map.NeedsOnlineUpdate = false; // V2 doesn't have a direct "NeedsUpdate" code yet
                }
                else
                {
                    // If map not found on server, mark as not submitted
