...
{
    /// <summary>
    ///     Fetches scores for <see cref="LeaderboardType.Rate"/> using API v2.
    /// </summary>
    public class ScoreFetcherRate : IScoreFetcher
    {
        public FetchedScoreStore Fetch(Map map, CancellationToken token = default)
        {
            try
            {
                ...
                var onlineScoresTask = System.Threading.Tasks.Task.Run(() => client.RetrieveScoreboardV2(map.Md5Checksum, OnlineScoreboard.Rate, mods));
                var pbResponseTask = System.Threading.Tasks.Task.Run(() => client.RetrievePersonalBestV2(map.Md5Checksum, OnlineManager.Self.OnlineUser.Id, OnlineScoreboard.Rate, mods));
