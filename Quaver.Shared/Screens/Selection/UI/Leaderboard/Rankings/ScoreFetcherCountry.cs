...
{
    /// <summary>
    ///     Fetches scores for <see cref="LeaderboardType.Country"/> using API v2.
    /// </summary>
    public class ScoreFetcherCountry : IScoreFetcher
    {
        public FetchedScoreStore Fetch(Map map, CancellationToken token = default)
        {
            try
            {
                ...
                if (!OnlineManager.Connected || OnlineManager.Client == null || OnlineManager.Self == null || !OnlineManager.IsDonator)
                    return new FetchedScoreStore(new List<Score>());

                var client = OnlineManager.Client;
                var country = OnlineManager.Self.OnlineUser.CountryFlag.ToUpper();

                // Parallel Fetching
                var mapInfoTask = System.Threading.Tasks.Task.Run(() => client.RetrieveMapInfoV2(map.Md5Checksum));
                var onlineScoresTask = System.Threading.Tasks.Task.Run(() => client.RetrieveScoreboardV2(map.Md5Checksum, OnlineScoreboard.Country, country: country));
                var pbResponseTask = System.Threading.Tasks.Task.Run(() => client.RetrievePersonalBestV2(map.Md5Checksum, OnlineManager.Self.OnlineUser.Id, OnlineScoreboard.Global));
